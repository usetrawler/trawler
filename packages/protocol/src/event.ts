import { z } from "zod";
import { FindingSchema, GoalOutcomeSchema, VerdictSchema } from "./finding.ts";
...
export const RunEventSchema = z.discriminatedUnion("type", [
  ...
  z.object({ ...base, type: z.literal("step"), step: z.number().int().nonnegative(), tool: z.string().nullable(), costUsd: z.number().nonnegative() }),
  ...
  z.object({ ...base, type: z.literal("verdict"), findingId: z.string(), verdict: VerdictSchema, observed: z.string() }),
  z.object({ ...base, type: z.literal("job_finished"), usage: JobUsageSchema, stoppedBy: z.string() }),
]);
