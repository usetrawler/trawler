...
test("a product that has had a run is not asked for the authorisation again; one without runs is", () => {
  const start = (authorisedBefore: boolean) => renderToStaticMarkup(createElement(StartRun, { projectId: "p1", personas: 2, keyHint: null, canManageKey: true, authorisedBefore }));
