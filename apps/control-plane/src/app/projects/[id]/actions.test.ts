...
const state = vi.hoisted(() => ({ member: null as Member | null, tenants: [] as string[], projectInWorkspace: true, keyChecks: 0, keysSaved: 0, keyStillStored: true, runsStarted: 0 }));
...
vi.mock("../../../credentials/credentials.ts", async (importOriginal) => ({
  ...
  keyStillStored: async () => state.keyStillStored,
}));
...
vi.mock("../../../runs/runs.ts", async (importOriginal) => ({
  ...
  startRun: async () => { state.runsStarted++; return { id: "run-1", number: 1 }; },
}));
...
test("a run whose saved key is still there starts and opens", async () => {
