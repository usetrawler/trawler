...
const { StartRun, modelsForKey, startRun } = await import("./start-run.tsx");
...
test("Start sends the form through the step that turns an update into a message", () => {
  draw();
  expect(react.served).toEqual([startRun]);
});
...
test("a refusal from the server is shown as it came", async () => {
  ...
  expect(await startRun({}, form())).toEqual({ error: "Confirm that you may test this product." });
  expect(actions.startRunAction).toHaveBeenCalledWith({}, expect.any(FormData));
});
...
test("a page left open across an update is told to reload, and keeps showing the key it showed", async () => {
  ...
  expect(await startRun({ keyHint, error: "The run could not start. Try again." }, form())).toEqual({ keyHint, error: "Trawler was updated since this page opened. Reload the page to start the run." });
});
...
test("the redirect to the new run, and any other failure, go on to the framework as before", async () => {
  ...
  await expect(startRun({}, form())).rejects.toBe(leaving);
  const failure = new TypeError("Failed to fetch");
  actions.startRunAction.mockRejectedValue(failure);
  await expect(startRun({}, form())).rejects.toBe(failure);
});
...
test("the model list of a page left open across an update says to reload instead of loading for ever", async () => {
  ...
  expect(setList).toHaveBeenLastCalledWith({ ok: false, error: "Trawler was updated since this page opened. Reload the page to load the models." });
});

test("a model list the server answers is passed on, and any other failure is not turned into the update message", async () => {
  const listed = { ok: true, provider: "openrouter", models: [], suggested: "" };
  actions.modelsForKeyAction.mockResolvedValue(listed);
  expect(await modelsForKey({})).toBe(listed);
  const failure = new TypeError("Failed to fetch");
