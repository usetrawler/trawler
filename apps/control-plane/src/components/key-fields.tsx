...
export function KeyFields({ apiKey, onKey, detected, chosen, onChoose, provider, baseUrl, onBaseUrl, required, aside, autoFocus, readOnly, errorId, baseUrlErrorId }: {
  apiKey: string; onKey: (key: string) => void; detected: Provider | null; chosen: Provider | null; onChoose: (provider: Provider) => void;
  provider: Provider | null; baseUrl: string; onBaseUrl: (url: string) => void; required: boolean; aside?: ReactNode; autoFocus?: boolean; readOnly?: boolean; errorId?: string; baseUrlErrorId?: string;
}) {
  const keyId = useId();
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        <input id={keyId} name="apiKey" type="password" autoComplete="off" spellCheck={false} required={required} autoFocus={autoFocus} readOnly={readOnly} aria-invalid={errorId ? true : undefined} aria-describedby={errorId} placeholder="sk-…, sk-ant-…, AIza…, sk-or-…" value={apiKey} onChange={(e) => onKey(e.target.value)} className={`${field} flex-1`} />
        {aside}
      </div>
      ...
      {apiKey.trim().length >= 20 && (
        <div className="grid gap-2 sm:grid-cols-[auto_1fr]">
          ...
          {provider === "custom" && <input name="baseUrl" type="url" placeholder="https://api.example.com/v1" aria-label="Base URL" readOnly={readOnly} aria-invalid={baseUrlErrorId ? true : undefined} aria-describedby={baseUrlErrorId} value={baseUrl} onChange={(e) => onBaseUrl(e.target.value)} className={field} />}
        </div>
      )}
